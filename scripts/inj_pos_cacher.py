"""
    python scripts/inj_pos_cacher.py <wallet_number> <market>
"""
import redis
import asyncio
#
...
with safe_import():
    ...
    from injective_utils.cachers import PositionCacher
...
async def main(host: str, port: int, db: int, market: str, subaccount_id: str):
    ...
    pc = PositionCacher(subaccount_id, market, redis_cli)
    await pc.run()
...
if __name__ == "__main__":
    ...
    print(f"Running Position Cacher on port {port} and db {db} for market {market}")
